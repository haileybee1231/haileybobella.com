$(document).ready(function() {
  ...
  var rerolls = 0;        //Collects reroll conditions
  var mordor = false;     //Determines whether or not to use Mordor algorithm
  var eyes;               //Number of eyes in hunt box
  var chance = $("#chance"); //Output for calculator

  chance.text(0 + '%');      //Initializes display to 0

  $("input[name=nazgul][choice=yes],input[name=armies][choice=yes],input[name=stronghold][choice=yes]").click(function() {     //Reroll function, adds 1 for each condition
    rerolls += 1;
  })
  $("input[name=nazgul][choice=no],input[name=armies][choice=no],input[name=stronghold][choice=no]").click(function() {     //Reroll function, subtracts 1 for each condition
    rerolls -= 1;
  })
  $("input[name=mordor][choice=yes]").click(function() {        //Controls Mordor parameter
    mordor = true;
  })
  $("input[name=mordor][choice=no]").click(function() {          //Revers Mordor to false if clicked
    mordor = false;
  })

//Main calculator function
  $("#calculate").click(function() {                           //Main calculate function
    eyes = parseInt($("#eyes").find(":selected").text());                //Sets number of eyes to currently selected
    var modifiers = $("#freePeoples").find(":selected").text().replace(/[+]/, ''); //Set modifiers to number of FP dice in box
    let possibilities = eyes > 0 ? Math.pow(6, (eyes + rerolls)) : 0;
    if (mordor) {                                              //If in Mordor
      chance.text(100 + '%');
    } else if (eyes === 0) {
      chance.text(0 + '%');
  ...
  function quote() {                               //Generates a random quote
    var randomQuote = quotes[Math.floor(Math.random()*quotes.length)];
    var text = randomQuote.quote;
    var author = randomQuote.author;
    console.log(text, author)
    $("#quote p").removeClass('hidden').text(text + ' -' + author);
  }

var quotes = [              //Quote array
    {
